...
<p align="center">
  <a href="https://mmmqaz.cn/releases/devops-v1.0.7-linux-x64.tar.gz"><strong>官网下载 v1.0.7</strong></a>
  ·
  <a href="https://github.com/xiaoxiao113213/agents-platform/releases/tag/v1.0.7">GitHub Release</a>
</p>
...
`v1.0.7` 支持 Linux x64 全新安装，也支持从 `v0.0.1` 及后续正式版本在原安装目录直接累计升级，无需补装中间版本。跨较大版本升级前，请先在数据库备份副本上核对业务数据。
...
./devops.sh images pull /tmp/devops-v1.0.7-linux-x64.tar.gz
```
...
```bash
./devops.sh upgrade --check /tmp/devops-v1.0.7-linux-x64.tar.gz
./devops.sh stop
./devops.sh upgrade /tmp/devops-v1.0.7-linux-x64.tar.gz
./devops.sh check
./devops.sh start
./devops.sh status
```
...
```bash
curl -fL -o devops-v1.0.7-linux-x64.tar.gz \
  https://mmmqaz.cn/releases/devops-v1.0.7-linux-x64.tar.gz

tar -xzf devops-v1.0.7-linux-x64.tar.gz
cd devops-v1.0.7
chmod +x devops.sh
./devops.sh install
```
...
当前正式版本：[`v1.0.7`](./docs/releases/v1.0.7.md)
