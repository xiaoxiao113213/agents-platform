...
export const releases: PlatformRelease[] = [
  {
    ...
    date: '2026-09-04',
    status: 'stable',
    title: '项目入口与长任务连接稳定性完善',
    summary: '精简项目与能力导航，完善多引擎模型连接、事项批量流转，并让远程开发引擎长任务在静默执行期间保持在线。',
    highlights: [
      '项目成为统一入口，左侧直接展示最近项目与全部项目；Skill 和 MCP 合并为能力广场并支持页内切换。',
      '模型连接可同时支持协作引擎和开发引擎，并分别配置平台内网地址与远程 Agent 外网地址。',
      '最近项目在临时网络故障时有限重试，失败后保留上次结果并等待用户主动恢复，避免持续请求。',
      '协作空间事项支持多选后批量流转到共同可用状态，并保留逐事项权限、版本和状态校验。',
      '远程开发引擎在长时间工具执行期间持续传递连接心跳，失败历史也会显示明确状态与处理时长。',
      '开发引擎与协作引擎使用同一消息布局，平台 MCP 和命令工具按真实类型展示。',
      '六个内置 Agent 与独立 IM Gateway 运行环境统一更新为 v1.0.54，SDK/CLI 稳定组合保持不变。',
    ],
    metrics: ['统一项目导航', '双引擎模型连接', '远程长任务不断线'],
    assetName: 'devops-v1.0.54-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后核对模型地址与远程长任务',
      summary: '确认模型连接的内外网地址，再用远程 Agent 执行一次包含工具等待的开发引擎任务，并检查最近项目恢复与批量状态流转。',
  ...
  },
  {
    ...
    status: 'stable',
    title: '四渠道 IM 机器人接入',
    summary: '个人微信、飞书、钉钉和企业微信通过独立 Gateway 接入平台 Agent，并具备身份审批、群聊控制和可靠投递。',
    highlights: [
      '新增独立 IM 机器人工作台，统一配置渠道、Agent、项目路由、执行引擎、API Key、模型和工作模式。',
      '支持个人微信腾讯 iLink 扫码，以及飞书、钉钉、企业微信官方长连接，运行时不依赖 dsh-im。',
      '支持私聊、受控群聊、多轮上下文、常用会话命令、问题补充和工具审批。',
      '未知身份默认进入审批，群聊需群组和成员获批，并且只有 @ 或回复机器人时才触发 Agent。',
      '支持文本、图片和文件收发，使用可靠发件箱完成失败重试与断线恢复。',
      'Linux 交付新增独立 Gateway 镜像和运维脚本，不新增公网端口。',
      '六个内置 Agent 运行环境统一更新为 v1.0.53，SDK/CLI 稳定组合保持不变。',
    ],
    metrics: ['4 类 IM 渠道', '身份与群聊安全控制', '可靠消息投递'],
    assetName: 'devops-v1.0.53-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '启用 Gateway 后逐渠道验收',
      summary: '先配置共享令牌和 WSS 地址，再使用官方测试租户验收连接、身份审批、群聊触发和消息重试。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    status: 'stable',
    title: '个人模型连接与完整对话轨迹',
    summary: '每位用户独立维护可用模型连接，并让双引擎对话完整呈现运行轨迹、时间与可靠的配置恢复。',
    highlights: [
      '每位用户都可以添加和维护自己的 API Key 与模型，支持个人、全局和指定用户三种可见范围。',
      'Agent 绑定、新建会话和定时会话统一校验当前用户可用的执行引擎、API Key 与模型。',
      '定时会话保存 API Key 和模型快照，实际执行时不受 Agent 默认配置后续变化影响。',
      '协作引擎和开发引擎都在主对话中展示完整思考与工具轨迹，支持逐项及全局展开或收起。',
      '每轮对话显示发送时间和处理时长，非当天消息包含日期，进行中时长实时更新。',
      '修复刷新会话时旧本地配置触发重复保存和会话设置保存失败的问题。',
      '旧 Workflow 能力正式退役，六个内置 Agent 运行环境统一更新为 v1.0.52。',
    ],
    metrics: ['个人模型连接', '双引擎完整轨迹', '会话配置稳定恢复'],
    assetName: 'devops-v1.0.52-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后核对模型范围与会话恢复',
      summary: '分别验证个人和指定用户模型连接，再刷新已有会话并完成两种引擎的思考与工具对话。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    date: '2026-09-02',
    status: 'stable',
    title: '六套 SDK 案例与项目工具权限完善',
    summary: '新增两套可独立部署案例，并统一 SDK、环境变量和终端在共享项目中的角色与资源边界。',
    highlights: [
      '开发者中心新增 Agent 工程工作台和策展引航员，六套完整案例均可下载、预览、搜索源码并复制到独立业务目录继续开发。',
      'Agent 工程工作台使用 Node.js、React 与 H2，策展引航员使用 Spring Boot、React 与 H2，页面交互和业务数据均可实际运行。',
      'SDK 和终端改为紧凑纯图标入口，SDK 位于终端左侧，并随工具栏一起展开或收起。',
      '角色管理新增独立 Agent SDK 权限，项目能力页面与服务端接口使用同一授权规则。',
      '共享成员可维护 Agent 环境变量；API Key 绑定和终端仍保持更严格的所有权与角色限制。',
      '案例交付 ZIP 使用可重复构建，并继续排除运行数据、密钥、依赖目录和构建缓存。',
      '六个内置 Agent 运行环境统一更新为 v1.0.51。',
    ],
    metrics: ['六套完整 SDK 案例', 'SDK 独立角色权限', '共享环境变量管理'],
    assetName: 'devops-v1.0.51-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后核对 SDK 与共享项目权限',
      summary: '确认六套案例均可访问，并分别用项目所有者和共享成员验证 SDK、环境变量、API Key 与终端边界。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    date: '2026-09-01',
    status: 'stable',
    title: '四套 SDK 案例与共享项目工具完善',
    summary: '统一四套可独立部署案例的账号初始化，并完善项目工具共享边界与 SDK 开发入口。',
    highlights: [
      'SDK 开发者中心新增智能法律工作台完整案例，四套案例均可下载、预览、搜索源码并复制到独立业务目录继续开发。',
      '四套案例首次启动自动创建 admin 管理员，默认密码为 11111111；用户修改密码后重启不会覆盖已有密码。',
      '案例登录后再配置平台 URL、SDK API Key、Agent、执行引擎、绑定 Key 和模型，适合客户独立部署初始化。',
      'Agent 项目工作区将 SDK 开发固定在终端左侧，减少工具折叠后查找成本。',
      '共享项目成员可以使用除终端外的项目工具；终端继续要求 Agent 所有权和终端权限，后端同时强制校验。',
      '案例交付 ZIP 使用可重复构建，并继续排除运行数据、密钥、依赖目录和构建缓存。',
      '六个内置 Agent 运行环境统一更新为 v1.0.50。',
    ],
    metrics: ['四套完整 SDK 案例', '默认管理员初始化', '共享项目工具完善'],
    assetName: 'devops-v1.0.50-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后核对案例登录与共享项目工具',
      summary: '启动任一 SDK 案例验证默认管理员，再由项目所有者和共享成员分别核对工作区工具与终端边界。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  ...
  },
  {
    ...
    date: '2026-08-28',
    status: 'stable',
    title: 'Linux 主服务内存可控且可诊断',
    summary: '按服务器容量配置主 JVM 内存，并通过统一校验、GC 日志和 OOM 堆转储提升生产可运维性。',
    highlights: [
      '可分别设置主服务 JVM 初始堆与最大堆，配置为 0 时可交由 Java 自动决定。',
      '启动、重启和前台运行统一使用经过校验的 JVM 参数。',
      '状态与环境检查直接显示最终生效的内存和诊断配置。',
      'GC 与安全点日志固定容量轮转，避免诊断日志无限增长。',
      '可在 OOM 时保留堆转储，并明确提示磁盘容量与敏感数据保护要求。',
      '六个内置 Agent 运行环境统一更新为 v1.0.45。',
      '从更早版本升级后继续自动分批恢复尚未补齐的对话正文。',
    ],
    metrics: ['JVM 内存可配置', '启动前严格校验', 'OOM 现场可分析'],
    assetName: 'devops-v1.0.45-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后合并四个 JVM 配置键',
      summary: '现场配置会继续保留；对比新版示例合并 JVM 内存与诊断参数，重启后通过 status 和 check 核对生效值。',
      guidePath: '/deploy#jvm-memory',
      action: '查看 JVM 配置与容量建议',
    },
  },
  {
    version: 'v1.0.44',
    ...
    status: 'stable',
    title: '对话历史完整保存并聚焦正文',
    summary: '平台稳定保存用户消息和模型正式回答，同时让进行中的最后一个工具状态保持清晰可见。',
    highlights: [
      '历史主对话隐藏已完成的思考、工具调用和工具结果，阅读时更聚焦有效问答。',
      '用户输入和模型正式回答由平台持久保存，上下文压缩后仍可继续查看更早对话。',
      '升级后自动分批补齐已有会话正文，重复扫描不会产生重复消息。',
      '对话执行期间展示最后一个工具状态，直到模型正式正文开始输出后自动收起。',
      '切换会话再返回时自动恢复进行中的草稿、最后工具和停止操作。',
      '加载更早消息直接批量读取平台历史，并尽量补足多轮可见正文。',
      '原始运行轨迹继续保留在运行详情中，排障能力不受影响。',
      'Agent 全部项目与指定项目分享继续覆盖项目内完整工具能力和新建项目授权。',
    ],
    metrics: ['正文持久保存', '会话切换恢复', '多轮批量加载'],
    assetName: 'devops-v1.0.44-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后核对历史与会话恢复',
      summary: '打开一段长历史并加载更早消息，再发起一次包含工具调用的对话，切换会话后确认运行状态可以恢复。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    date: '2026-08-27',
    status: 'stable',
    title: 'Agent 分享覆盖完整项目工作流',
    summary: '按全部项目或指定项目分享 Agent，让成员在授权范围内完成对话、工具和协作空间工作。',
    highlights: [
      '全部项目分享自动覆盖当前及后续创建的项目，指定项目分享保持清晰隔离。',
      '获授权成员可使用项目内对话、文件、Skill、MCP、Workflow、应用和项目服务。',
      '具备项目创建权限的成员创建新项目后会自动获得对应授权，无需管理员再次补配。',
      '项目重命名、删除和 Agent 全局配置继续按所有者、创建者与最高管理员边界控制。',
      '绑定的 Issue 协作空间同步提供事项与资料操作能力。',
      'Developer SDK 与业务工作台案例增强浏览器兼容性和本地服务管理体验。',
    ],
    metrics: ['完整项目工具权限', '新项目自动授权', '协作空间继承'],
    assetName: 'devops-v1.0.43-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后核对分享范围',
      summary: '使用被分享账号分别检查全部项目和指定项目模式，并验证项目工具、对话与协作空间操作。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  ...
  },
  {
    ...
    highlights: [
      '用户要求文件时必须提交真实文件产物，平台页面和预览地址不再替代附件。',
      '文件已经生成但尚未交付时，Agent 会继续完成产物提交。',
      '大文件传输、校验和固化流程优化，减少同账号其他操作排队。',
    ...
    assetName: 'devops-v1.0.39-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后复验一次文件交付',
      summary: '先导入本版本运行环境并完成累计升级，再让 Agent 生成并交付一个真实文件，核对附件与下载文件名。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    date: '2026-08-25',
    status: 'stable',
    title: 'Skill 分类与项目协作效率升级',
    summary: '用领域分类组织 Skill，让最近项目、能力目录、Agent 管理和协作空间更容易查找、判断与继续工作。',
    highlights: [
      'Skill 分类支持维护、智能归类和人工校准，已有 Skill 的分类不能被误删。',
      '不同领域主题可独立控制开放的 Skill 分类，项目添加能力时只展示当前成员可用内容。',
      '项目添加 Skill 支持分类筛选，能力广场 Skill 与 MCP 统一使用更易浏览的卡片目录。',
      '最近项目按活动时间加载全部可访问项目，每批 10 个，并明确任务执行和待查看状态。',
      '我的 Agent 可以直接重命名，个人主 Agent 保持清晰标识。',
      '协作空间移至开始导航，组合筛选支持多选并正确反映授权来源。',
      '平台配置与自动化统一收纳到系统管理，并修复了已知问题。',
      '本版本不新增域名、DNS、证书、公开端口或生产配置键。',
    ],
    metrics: ['领域 Skill 分类', '最近项目分批加载', '协作筛选多选'],
    assetName: 'devops-v1.0.38-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后复验分类与协作入口',
      summary: '先导入本版本运行环境并完成累计升级，再检查一个领域分类、一个最近项目和一次协作空间组合筛选。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    date: '2026-08-24',
    status: 'stable',
    title: '跨模型用量与真实项目能力核对',
    summary: '统一查看全部模型的 Token 使用情况，并让项目 Skill、实时文件与独立终端更准确地反映 Agent 工作现场。',
    highlights: [
      'Token 用量支持近 3 天、近 7 天、近 30 天和自定义日期，并统一摘要、趋势、分布和导出口径。',
      '默认按用户聚合所有模型和 API Key，也可切换模型、API Key 与明细维度。',
      '项目内既有 SKILL.md 按真实目录和元数据识别，贡献统计与使用反查保持一致。',
      '文件弹窗中的 Skill 元数据与 Markdown 正文使用 Skill 中心同一套预览。',
      '终端固定在独立浏览器标签页打开，不再占用工作区右侧空间。',
      '实时目录树支持整体刷新和单目录刷新，并修复了已知问题。',
      '本版本不新增域名、DNS、证书、公开端口或生产配置键。',
    ],
    metrics: ['全部模型汇总', '真实 Skill 贡献', '实时目录刷新'],
    assetName: 'devops-v1.0.37-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后复验用量、Skill 与工作区',
      summary: '先导入本版本运行环境并完成累计升级，再核对一个统计周期、一个既有 Skill 和一次目录刷新。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
  {
    ...
    status: 'stable',
    title: '项目能力与协作权限完整闭环',
    summary: 'Agent 项目默认进入已有工作区，Skill、MCP、文件预览与协作空间授权形成清晰、可追踪的一体化体验。',
    highlights: [
      '进入 Agent 后默认打开第一个可访问项目，无项目时先完成创建再开始对话。',
      'Skill 与 MCP 使用独立能力列表，支持真实使用统计和项目内已安装能力反查。',
      '工作区统一展示相对路径，并支持 Markdown、代码、HTML、图片、PDF、音频和视频预览。',
      '协作空间支持成员授权、Agent 分享继承、撤销和不可变授权日志。',
      '外部 Agent 可以按页面指引接入协作空间，共用事项、资料和处理记录。',
      '领域主题可维护平台名称并在登录、导航和业务页面统一生效。',
      '工具栏、Skill 预览、产物计数和对话完成提醒等交互同步修复。',
      '修复 Windows 远程 Agent 创建单端点托管应用时可能误判端口未监听的问题。',
      '本版本不新增域名、DNS、证书、公开端口或生产配置键，并修复了已知问题。',
    ],
    metrics: ['项目默认直达', '多来源权限审计', '浏览器原生预览'],
    assetName: 'devops-v1.0.36-linux-x64.tar.gz',
    guidedInstall: true,
    upgradeNotice: {
      title: '升级后复验项目能力与协作授权',
      summary: '先导入本版本运行环境并完成累计升级，再检查一个已有项目、一项 Skill/MCP 和一条协作空间授权记录。',
      guidePath: '/deploy#verify',
      action: '查看升级验证步骤',
    },
  },
